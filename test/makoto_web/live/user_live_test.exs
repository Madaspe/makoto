defmodule MakotoWeb.OwnerLiveTest do
  use MakotoWeb.ConnCase

  import Phoenix.LiveViewTest
  import Makoto.AccountsFixtures
  ...
  describe "Index" do
    ...
    test "updates user in listing", %{conn: conn, user: user} do
      ...
      assert_patch(index_live, Routes.owner_index_path(conn, :edit, user))
  ...
  describe "Show" do
    ...
    test "displays user", %{conn: conn, user: user} do
      {:ok, _show_live, html} = live(conn, Routes.owner_show_path(conn, :show, user))
    ...
    test "updates user within modal", %{conn: conn, user: user} do
      {:ok, show_live, _html} = live(conn, Routes.owner_show_path(conn, :show, user))

      assert show_live |> element("a", "Edit") |> render_click() =~
               "Edit User"

      assert_patch(show_live, Routes.owner_show_path(conn, :edit, user))
      ...
      {:ok, _, html} =
        ...
        |> form("#user-form", user: @update_attrs)
        |> render_submit()
        |> follow_redirect(conn, Routes.owner_show_path(conn, :show, user))
