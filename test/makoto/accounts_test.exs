defmodule Makoto.AccountsTest do
  use Makoto.DataCase

  alias Makoto.Accounts

  import Makoto.AccountsFixtures
  alias Makoto.Accounts.{User, UserToken}
  ...
  describe "get_user_by_id!/1" do
    ...
    test "returns the user with the given id" do
      %{id: id} = user = user_fixture()
      assert %User{id: ^id} = Accounts.get_user_by_id!(user.id)
    end
  end
  ...
  describe "apply_user_email/3" do
    ...
    test "applies the email without persisting it", %{user: user} do
      ...
      assert user.email == email
      assert Accounts.get_user_by_id!(user.id).email != email
    end
  end
